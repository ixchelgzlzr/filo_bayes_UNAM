...
### El objetivo final: $$P(A\middle|B)$$

En realidad lo que nos interesa como estadísticos bayesianos es saber el resultado final para Silvia ¿acabará enamorandose?, en términos de probabilidad nos interesa lo que llamamos la probabilidad a posteriori, es decir  $$P(A\middle|B)$$ = la probabilidad del carisma de Silvia dadas las citas exitosas durante el reality show. En algunos contextos la probabilidad a posteriori se le conoce como la actualización de las probabilidades porque conocemos más del carisma de Silvia cada vez que tiene una cita en el reality. 
...
Finalmente vamos a calcular $$P(A\middle|B)$$ o en el nuevo lenguaje de nuestras variables aleatorias $$P(\theta\middle|Datos)$$ es la distribución posterior del carisma de Silvia
...
3. Calcula la distribución posterior $$P(\theta_{old}\middle|Datos)$$
4. Proponer una distribución aleatoria $$g(\theta)$$ para obtener un nuevo valor $$\theta_new$$
5. Calcula la distribución posterior $$P(\theta_{new}\middle|Datos)$$
6. Calcula los momios $$momios=\frac{P(\theta_{new}\middle|Datos)}{P(\theta_{old}\middle|Datos)}$$
